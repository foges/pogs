...
# Register POGS with CVXPY when this module is imported
def _register_cvxpy_solver():
    """Register POGS as a CVXPY solver."""
    try:
        import cvxpy
        ...
        class POGS(ConicSolver):
            """CVXPY solver interface for POGS (basic cone form support)."""

            MIP_CAPABLE = False
            SUPPORTED_CONSTRAINTS = [cvxpy.Zero, cvxpy.NonNeg]

            def name(self):
                return "POGS"

            def import_solver(self):
                import pogs  # noqa: F401

            def solve_via_data(self, data, warm_start, verbose, solver_opts, solver_cache=None):
                # For now, recommend using pogs_solve() for best performance
                raise NotImplementedError(
                    "Use pogs_solve(problem) instead for best performance. "
                    "Direct cone-form solving via CVXPY is not yet fully supported."
                )
        ...
        pass  # Registration failed, but don't break import


_register_cvxpy_solver()
