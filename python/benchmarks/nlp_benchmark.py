...
def load_20newsgroups(n_samples=500, n_features=1000):
    ...
    try:
        ...
        newsgroups_pos = fetch_20newsgroups(
            subset="train", categories=categories_pos, remove=("headers", "footers", "quotes")
        )
        newsgroups_neg = fetch_20newsgroups(
            subset="train", categories=categories_neg, remove=("headers", "footers", "quotes")
        )

        # Combine and subsample
        texts = newsgroups_pos.data[: n_samples // 2] + newsgroups_neg.data[: n_samples // 2]
        labels = [1] * min(len(newsgroups_pos.data), n_samples // 2) + [-1] * min(
            len(newsgroups_neg.data), n_samples // 2
        )
...
def solve_logistic_pogs(A, b, lambd, verbose=False):
    ...
    t0 = time.perf_counter()
    result = solve_logistic(
        A, b, lambd=lambd, abs_tol=1e-4, rel_tol=1e-4, max_iter=5000, verbose=5 if verbose else 0
    )
    solve_time = time.perf_counter() - t0
...
def run_benchmark():
    ...
    # Synthetic spam
    X, y, _name = load_spam_dataset(n_samples=500, n_features=500)
    if X is not None:
    ...
    for dataset_name, X, y, lambd in datasets:
        ...
        for solver in solvers:
            ...
            if x_sol is not None and acc is not None:
                ...
                solver_results[solver] = {"time": t, "accuracy": acc, "nnz": nnz, "status": status}
            else:
                print(f"FAILED ({status})")
                solver_results[solver] = {"time": t, "accuracy": None, "status": status}

        results.append({"dataset": dataset_name, "size": (m, n), "solvers": solver_results})
