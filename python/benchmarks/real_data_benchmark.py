...
try:
    from pogs_graph import FunctionObj, Function, _solve_graph_form, solve_lasso
    HAS_POGS = True
except ImportError:
    HAS_POGS = False
...
def load_sklearn_datasets():
    ...
    try:
        from sklearn.datasets import load_diabetes, load_wine, load_breast_cancer, load_digits
...
def load_sparse_text_data():
    ...
    try:
        ...
        y = np.array([1 if 'sci' in newsgroups.target_names[t] else -1
                      for t in newsgroups.target])

        sparsity = 1 - X.nnz / (X.shape[0] * X.shape[1])
        datasets.append((f'20newsgroups ({X.shape[0]}x{X.shape[1]}, {100*sparsity:.0f}% sparse)',
                        X, y, sparsity))
        ...
        try:
            ...
            sparsity = 1 - X.nnz / (X.shape[0] * X.shape[1])
            datasets.append((f'SparseClassification ({X.shape[0]}x{X.shape[1]}, {100*sparsity:.0f}% sparse)',
                            X, y, sparsity))
...
def load_libsvm_dataset(name='a1a'):
    ...
    datasets = {
        ...
        'news20': ('https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/binary/news20.binary.bz2', 19996, 1355191),
        'rcv1': ('https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/binary/rcv1_train.binary.bz2', 20242, 47236),
        'real-sim': ('https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/binary/real-sim.bz2', 72309, 20958),
    }

    if name not in datasets:
        return None, None, f"Unknown: {name}"

    url, expected_samples, expected_features = datasets[name]
