...
def solve_mpc_pogs(A, B, Q, R, x_min, x_max, u_min, u_max, x0, horizon):
    ...
    for t in range(horizon + 1):
        for i in range(n_state):
            ...
            g.append(FunctionObj(Function.kIndBox01, scale, x_min[i]*scale, 1.0, 0.0, Q_diag[i]/2))
    ...
    for t in range(horizon):
        for i in range(n_input):
            ...
            g.append(FunctionObj(Function.kIndBox01, scale, u_min[i]*scale, 1.0, 0.0, R_diag[i]/2))

    t0 = time.perf_counter()
    result = _solve_graph_form(C, f, g, abs_tol=1e-4, rel_tol=1e-4,
...
def run_benchmark():
    ...
    for n_state, n_input, horizon in configs:
        ...
        A, B, Q, R, x_min, x_max, u_min, u_max, x0 = generate_mpc_problem(
            n_state, n_input, horizon
        )
        ...
        for solver in solvers:
            if solver == 'POGS':
                if HAS_POGS:
                    val, t, status = solve_mpc_pogs(A, B, Q, R, x_min, x_max,
                                                     u_min, u_max, x0, horizon)
                else:
                    val, t, status = None, None, 'unavailable'
            else:
                val, t, status = solve_mpc_cvxpy(A, B, Q, R, x_min, x_max,
                                                  u_min, u_max, x0, horizon, solver)
